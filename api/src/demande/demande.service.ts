...
await this.userModel.updateOne({_id: createDemandeDto.users},
 ...
 async findOne(id: string):Promise<IDemande> {
     ...
     const data = await this.demandeModel.findById(id).populate({path: 'products', populate:[{path:'user'}]})
     if (!data) {
 ...
 async remove(id: string) {
  ...
  async findAllDemandeByuser(UserId: string):Promise<IDemande[]>
   {
    const data= await this.demandeModel.find({users :UserId}).populate('products')
    if (!data || data.length ==0 ){
   return null
    }
      return data;
   ...
   return null
    ...
    async findAllDemandeByVendorAndState(Vendorid: string, state : boolean):Promise<IDemande[]>{
